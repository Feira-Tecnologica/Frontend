...
username.addEventListener("input", function(){
    if (username.value.length < 4) {
        usernameError.textContent = "*O nome de usuário deve ter ao menos 8 caracteres."
    }else if(username.value.length > 20){
        usernameError.textContent = "*O nome de usuário deve ter menos que 20 caracteres.";
    }else {
...
password.addEventListener("input", function(){
    if (password.value.length < 8) {
        ...
        passwordError.textContent = "*A senha deve ter menos que 20 caracteres."
    }else {
...
lgEmail.addEventListener("input", function(){
    if (!checkEmail2(lgEmail.value)) {
        lgEmailError.textContent = "*Email inválido."
    }else {
...
lgPassword.addEventListener("input", function(){
    if (lgPassword.value.length < 8) {
        ...
        lgPasswordError.textContent = "*A senha deve ter mennos de 20 caracteres."
    }else {
...
function checkRequiredLg(inputArr2) {
    ...
    inputArr2.forEach(function(input){
        if (input.value.trim() === '') {
            showError2(input, `*${getFieldNameLg(input)} Por favor, insira os dados nesse campo`)
            isRequiredLg = true
        }else {
            showSuccess2(input)
        }
